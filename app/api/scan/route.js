...
import oversoldSymbols from '@/app/lib/symbols_oversold.json' assert { type: 'json' };
...
export async function GET(request) {
  ...
  const universeMap = {
    ...
    'agent-oversold': oversoldSymbols,
  ...
  try {
    ...
    else if (tab === 'agent-oversold') {
      results = (await batchFetch(oversoldSymbols))
        .filter(hasMinVolume)
        .sort((a, b) => (a.changePercent ?? 0) - (b.changePercent ?? 0)); // weakest first
